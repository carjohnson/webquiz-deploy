...
async function downloadToFile(url, dest) {
  ...
  // since SEG/DICOM files can be large.
  const fileStream = fsSync.createWriteStream(dest);
  try {
    await pipeline(Readable.fromWeb(response.body), fileStream);
  } catch (err) {
    // Clean up a partially-written file on failure so a retry doesn't
    // silently "succeed" with a truncated file.
    await fsPromise.rm(dest, { force: true });
    throw err;
  }
}
...
exports.segfile_get = asyncHandler(async (req, res, next) => {
  try {
    ...
    // Only allow serving files that actually live under the app's own
    // /outputs directory. Without this, ?path= could be used to read any
    // file the server process can access (e.g. ../../.env).
